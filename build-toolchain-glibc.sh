...
# 配置参数
export TARGET="${TARGET:-riscv64-linux-gnu}"

case "$TARGET" in
    aarch64-linux-gnu)    CROSS_KERNEL_NAME=arm64 ;;
    riscv64-linux-gnu)    CROSS_KERNEL_NAME=riscv ;;
    loongarch64-linux-gnu) CROSS_KERNEL_NAME=loongarch ;;
    x86_64-linux-gnu) CROSS_KERNEL_NAME=x86 ;;
    *) error "不支持的架构: $TARGET" ;;
esac
...
mkdir -p "$LOG_DIR"/{binutils,gcc,glibc} || error "无法创建日志目录"
